...
export function Select({label,options,formHandler,fieldName,required=false}:SelectProps){
    ...
    return <>
        <div className="">
            ...
            <select {...formHandler.register(fieldName,{required})} id={fieldName} name={fieldName} className={`block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 ${isRequiredError? "text-red-600 outline-red-600":""}`}>
